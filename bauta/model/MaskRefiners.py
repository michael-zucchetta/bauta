...
class MaskRefiners(nn.Module):
    ...
    def forward(self, embeddings):
        outputs = []
        input_image_size, predicted_masks, masks_embeddings, embeddings_merged, embeddings_2, embeddings_4, embeddings_8 = embeddings
        for class_index in range(self.classes):
            predicted_mask = predicted_masks[:,class_index:class_index+1,:,:]
            mask_embeddings = masks_embeddings[:,class_index:class_index+1,:,:].squeeze(1)
