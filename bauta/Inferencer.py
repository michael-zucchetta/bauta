...
class Inferencer():
            ...
            for class_index in refiner_dataset[batch_index].keys():
                for connected_component in refiner_dataset[batch_index][class_index]:
                    ...
                    if self.visual_logging:
                        ...
                        cv2.waitKey(0)
                    predicted_refined_mask = F.upsample(predicted_mask, size=(original_image.size()[1], original_image.size()[2]), mode='bilinear')
                    inference_result = InferenceResult(\
    ...
    def inferenceOnImage(self, model, original_input_image):
        input_image, new_height, new_width = self.image_utils.paddingScale(original_input_image)
        if self.visual_logging:
